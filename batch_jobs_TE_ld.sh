...
do
    ...
    do
        ...
        do
            for cv in {0..8}
            do
                jobid="TE_v2_cv"$cv"_csTE_"$csTE"_aE_"$aE"_ld_"$ld
                ...
                echo '#PBS -l mem=16g,walltime=5:00:00'>>subjob.bash
                ...
                echo 'python -m ae_model_train --batchsize 200 --cvfold '$cv' --alpha_T 1.0 --alpha_E '$aE' --alpha_M '$aE' --lambda_TE '$csTE' --latent_dim '$ld' --n_epochs 1500 --n_steps_per_epoch 500 --ckpt_save_freq 2000 --n_finetuning_steps 500 --run_iter 0 --model_id v2 --exp_name TE_Patchseq_Bioarxiv'>>subjob.bash
                echo '...'
                sleep 1
                wait
                qsub subjob.bash
