...
def get_merged_ordered_classes(data_labels, htree_file='../data/proc/dend_RData_Tree_20181220.csv', n_required_classes=30):
    ...
    #Get inhibitory subtree from the full hierarchical taxonomy
    ...
    if n_required_classes > np.unique(data_labels).size: 
        print("WARNING: n_required_classes exceeds number of classes in data_labels")
    n = 0
    ...
    if n_remain_classes != n_required_classes:
        print(f'WARNING: Merges for exactly {n_required_classes} classes not available')
        n = np.max([n-1,0])

    new_data_labels = do_merges(labels=data_labels.copy(), list_changes=L, n_merges=n, verbose=False)
