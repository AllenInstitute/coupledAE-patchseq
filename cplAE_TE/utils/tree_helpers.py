...
def get_valid_partitions(current_node_list, C_list, P_list, valid_partitions):
    ...
    Args:
        ...
        valid_partitions: Stores combinations of all valid partitions
    """
    current_node_list.sort(key=lambda x: x.name)
    valid_partitions.append(sorted([node.name for node in current_node_list]))
    for node in current_node_list:
        ...
        if len(children_node_list) > 0:
            ...
            if sorted([node.name for node in current_node_list_copy]) not in valid_partitions:
                valid_partitions = get_valid_partitions(current_node_list_copy,
                                                        C_list=C_list, P_list=P_list,
...
def simplify_tree(pruned_subtree,skip_nodes=None, verbose=True):
